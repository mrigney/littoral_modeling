...
The exact file layout -- property order, types, face encoding, byte sizing --
is in [16.6](#166-what-is-actually-in-the-ply).
...
### 16.6 What is actually in the PLY

The header from a real export, verbatim:

```
ply
format binary_little_endian 1.0
comment generated by pywave at t = 0.0 s
comment scene test_lake seed 20260801 mesh_dx 0.125 m
element vertex 291023
property float x
property float y
property float z
property float nx
...
property float foam
property float mss
property float wdir_x
property float wdir_y
...
Then the vertex block, then the face block, both packed with no padding.

#### Vertex properties

Thirteen `float32` per vertex, in this order. Everything is little-endian.

| # | Property | Units | Range | Meaning |
|---|---|---|---|---|
| 0–2 | `x`, `y`, `z` | m | scene coords | **Displaced** position. Z up, `z` about `water_level`, horizontal already offset by `choppiness × displacement`. |
| 3–5 | `nx`, `ny`, `nz` | — | unit | Analytic normal from the spectral slopes. `nz > 0` always. |
| 6 | `aniso` | — | ~1.0–1.3 | Crosswind / upwind slope variance ratio. |
| 7 | `depth` | m | −margin … max | Still-water depth at the **rest** position. Negative on the landward margin. |
| 8 | `foam` | — | 0–1 | Foam coverage fraction. |
| 9 | `mss` | — | >0 | Sub-mesh mean square slope. `α_Beckmann = sqrt(mss)`. |
| 10–11 | `wdir_x`, `wdir_y` | — | unit | Local **refracted** wave direction. |
| 12 | `wetness` | — | 0–1 | Submergence duty cycle in the swash band. |

**Key the channels by name, not by index.** Position and normal are always first
and in that order, but the channels after them are written in *alphabetical*
order (`write_ply` does `sorted(mesh.channels)`). Add a channel later and every
positional index after it shifts. A reader that looks up `"mss"` in the header
keeps working; one that hardcodes offset 9 does not.

#### Faces

`property list uchar int vertex_indices` — one `uint8` count (always 3) followed
by three `int32` indices, so **13 bytes per triangle**, unpadded. Winding is
counter-clockwise seen from +Z, which agrees with the analytic normal.

#### Size

```
bytes = header + n_vertices × 13 × 4 + n_faces × 13
```

The shipped lake window: 476 + 291,023 × 52 + 579,600 × 13 = 22,668,472 B
= 21.62 MiB, which is the file byte for byte. Roughly **52 bytes per vertex plus
26 per vertex of triangles**, so about 78 B/vertex in practice — useful for
sizing a frame sequence before generating one.
...
data["mss"]        # (V,) float32, keyed by name
data["x"]          # ...
faces              # (F, 3) int32
```

`read_ply` is deliberately narrow — it reads what `write_ply` produces and
nothing else. Its job is to let the round-trip test assert the channels survive,
not to be a general PLY parser. For anything beyond that use `plyfile`,
`open3d` or `trimesh`, all of which read this file; note that some libraries
silently drop unknown vertex properties, which is the failure mode the whole
format choice exists to avoid, so check before trusting one.

#### Into Mitsuba

The expectation is that Mitsuba's PLY loader exposes these as mesh attributes
reachable from a custom BSDF as `mesh_attribute` textures, conventionally named
`vertex_<property>` — so `vertex_mss`, `vertex_foam` and so on.

**That has not been verified against Mitsuba here**, because Mitsuba is a Phase 7
dependency and is not installed. Cookbook section 6.6 calls for confirming it
with a trivial test mesh as the first thing Phase 7 does, and it is right: it is
a five-minute check that either de-risks the whole phase or tells you the
delivery format needs rethinking before any C++ is written. The round-trip test
